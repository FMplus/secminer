<?php 
    ...
    if(isset($_SESSION['id'])){
        ...
        $count = $order_db -> buyer_order_count($id);
        $begin = $PAGE_ID*$PAGE_MAX_NUM;
        $PAGE_NUM = (integer)($count/$PAGE_MAX_NUM);
        if($count%$PAGE_MAX_NUM > 0){
            $PAGE_NUM++;
        }
        $order_info_id = $order_db -> fetch_buyer_order_list( $id, $begin,$PAGE_MAX_NUM);
...
<tr>
	<td><a href = "order.php?oid=<?= $value -> id?>"><?= $value -> id ?></a></td>
	<td><a href = "goodsinfo.php?id=<?= $value -> gid?>" target ="_blank"><?= $value -> gid ?></a></td>
	<td><?= $value -> number ?></td>
	<td><?= $value -> price ?></td>
...
<?php if($PAGE_NUM <= $MAX_PAGE_ID){
        for($i = 1;$i <= $PAGE_NUM;$i++){
            ...
            <?=$i?>&nbsp
 <?php      }else{?>
            <a href = "buyerorder.php?page=<?= $i-1?>"><?=$i?></a>&nbsp
<?php       }
        }?>
        >
<?      }else{
...
<?php       }
        ...
        ...
        <a href = "buyerorder.php?page=<?= $PAGE_NUM-1?>"><?=$PAGE_NUM?></a>&nbsp
        >
<?php }?>
&nbsp
<input type = "text" name = "pageid" id = "pageid" value = '<?=$PAGE_ID+1?>' align = 'right' size = '3'/>/<?=$PAGE_NUM?>&nbsp
<input type = "button" onclick = "jump('buyerorder.php?page=',pageid.value - 1)" value = "Go"/>
</div>
